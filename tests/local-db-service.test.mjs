...
import { databasePath } from "../electron/db-store.js";
...
const task = (title = "HTTP task") => {
  ...
  return {
    ...
    status: "not_started", criticality: "medium", recurrence: "none", recurrenceEnd: null,
    completedAt: null, createdAt: now, updatedAt: now,
  };
};

test("loopback service exposes the physical SQLite store over HTTP", async () => {
