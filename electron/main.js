...
function newWindow(overrides = {}) {
  return new BrowserWindow({
    width:1440, height:900, minWidth:900, minHeight:640,
...
async function runRendererSmoke() {
  ...
  const result=await win.webContents.executeJavaScript(`(async()=>{
    ...
    const id=await window.todoDb.saveTask({
      ...
      backlogOrder:null,status:"not_started",criticality:"medium",recurrence:"none",recurrenceEnd:null,completedAt:null,createdAt:now,updatedAt:now
    });
...
async function runHttpRendererSmoke() {
  ...
  const win=newWindow({show:false,webPreferences:{contextIsolation:true,nodeIntegration:false,sandbox:true}});
  const errors=collectErrors(win);
  ...
  try{
    ...
    const result=await win.webContents.executeJavaScript(`(async()=>{
      ...
      const basicId=await db.saveTask({
        title:"HTTP renderer smoke",description:"browser-style integration",
        startsAt:now,endsAt:new Date(Date.now()+3600000).toISOString(),
        backlogOrder:null,status:"not_started",criticality:"medium",recurrence:"none",recurrenceEnd:null,
        completedAt:null,createdAt:now,updatedAt:now
      });
      const basicSaved=await db.getTask(basicId);
      await db.deleteTask(basicId);

      const backlogId=await db.saveTask({
        title:"Backlog drag smoke",description:"drag to calendar",
        startsAt:null,endsAt:null,backlogOrder:0,status:"not_started",criticality:"high",
        recurrence:"none",recurrenceEnd:null,completdAt:null,createdAt:now,updatedAt:now
      });
      await window.todoRenderApp();
      await new Promise(r=>setTimeout(r,120));

      const card=document.querySelector('[data-backlog-task="'+backlogId+'"]');
      const modeButton=document.querySelector([name="mode"]);
      const modeButtonParsed=document.querySelector('[data-mode="day"]');
      modeButtonParsed?.click();
      await new Promise(r=>setTimeout(r,120));
      const slot=document.querySelector('.time-slot[data-hour="14"]');
      if(!card||!slot){
        return{
          hasDesktopBridge:!!window.todoDb,(hasCalendar:!!document.querySelector(".calendar-panel")),hasBacklog:!!document.querySelector("[data-backlog]"),cardFound:!!card,slotFound:!!slot,info,basicSavedTitle:basicSaved?.title
        };
      }
      ...
      const scheduled=await db.getTask(backlogId);
      const expectedDate=slot.dataset.date;
      const expectedHour=Number(slot.dataset.hour);
      const start=new Date(scheduled?.startsAt);
      const end=new Date(scheduled?.endsAt);
      const stillInBacklog=!!document.querySelector('[data-backlog-task="'+backlogId+'"]');

      await db.deleteTask(backlogId);
      ...
      return{
        ...
        info,
        basicSavedTitle:basicSaved?.title,
        scheduledId:scheduled?.id,
        backlogOrder:scheduled?.backlogOrder,
        scheduledDate:scheduled?.startsAt?.slice(0,10),
    ...
    const ok=
      ...
      result.hasBacklog &&
      result.info &&
      result.basicSavedTitle==="HTTP renderer smoke" &&
      result.scheduledId &&
      result.backlogOrder===null &&
      result.scheduledDate===result.expectedDate &&
      result.scheduledHour===result.expectedHour &&
      result.durationMs===3600000 &&
      result.stillInBacklog===false &&
      errors.length===0;

    if(!ok) throw new Error(`HTTP renderer/backlog smoke failed: ${JSON.stringify({...result,errors})}`);
    console.log(`TodoApp HTTP smoke OK: UI + loopback SQLite + backlog drag/drop schema ${result.info.schemaVersion} @ ${dbInfo.filePath}`);
  }finally{
    win.destroy();
