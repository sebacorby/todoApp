...
async function reloadColors(){COLORS={...DEFAULT_COLORS,...await getSetting("criticalityColors",{})};render()}window.todoReloadColors=reloadColors;
async function render(){
 $$(".nav-item").forEach(b=>b.classList.toggle("active",b.dataset.view===view));$("#view-title").textContent=view==="calendar"?"Calendario":"Dashboard";$("#eyebrow").textContent=view==="calendar"?"PLANIFICACIÓN":"RESUMEN";
...
$("#task-form").addEventListener("submit",async e=>{
 ...
 const task={...(old||{}),title,description:$("#task-description").value.trim(),startsAt,endsAt,status,criticality:$("#task-criticality").value,recurrence,recurrenceEnd,completedAt:status==="completed"?(old?.completedAt||now):null,createdAt:old?.createdAt||now,updatedAt:now};if(id)task.id=id;await saveTask(task);closeModal();toast(id?"Tarea actualizada":"Tarea creada");render()
});
